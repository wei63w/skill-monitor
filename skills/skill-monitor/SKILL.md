...
name: skill-monitor
description: >-
  Monitor and analyze how often local Agent Skills are used. Use whenever the
  user asks about skill usage, skill frequency, which skills are unused, skill
  call counts, backfilling skill history from transcripts, or setting up skill
  usage tracking in a project. Also use when the user says skill 监控, skill
  统计, skill 频率分析, or 回填 skill 使用记录.
---
...
| `analyze [--write]` | Usage frequency table → stdout and optional `data/report.md` |
| `summary` | Print aggregated `data/summary.json` |

## Data layout

- `data/events.jsonl` — one JSON event per line
- `data/summary.json` — aggregated counts
...
```json
{"ts":"...","skill":"name","path":".../SKILL.md","source":"hook|backfill|manual","sessionId":null,"tool":"cursor"}
```
